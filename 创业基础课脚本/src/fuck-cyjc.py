...
# 类似于#!/bin/bash
# 上面一行添加解释器
# 如果不加的话，使用python3 fuck-cyjc.py
# 加上的话，赋予文件x权限，可以像脚本一样直接执行
# 如果加上解释器，且外面命令行使用python3，且内部指定了解释器，外部优先

# logo可以不用自己敲出来，可以自动生成的啦
# 利用命令行打印出字符图案：https://blog.csdn.net/u014636245/article/details/83661559#commentBox
# 前几天，我使用print打印字符串的时候，使用print{f"aaa{var}""}
# 下面这样的写法也值得借鉴，用{!s}占位，打印的时候用str.format(var)替换
__LOGO__ = \
'''
